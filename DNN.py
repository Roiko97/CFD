...
class DNN(nn.Module):
    ...
    """
    def __init__(self, in_dims, out_dims, emb_size, community_dim, time_type="cat", norm=False, act_func='tanh', dropout=0.):
        ...
        if self.time_type == "cat":
            in_dims_temp = [self.in_dims[0] + self.time_emb_dim + community_dim] + self.in_dims[1:]
    ...
    def forward(self, x, timesteps, cluster_center):
        time_emb = timestep_embedding(timesteps, self.time_emb_dim).to(x.device)
        emb = self.emb_layer(time_emb)
        if self.norm:
            x = F.normalize(x, dim=-1)
        x = self.dropout(x)
        h = torch.cat([x, emb, cluster_center], dim=-1)
