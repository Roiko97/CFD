...
class GaussianDiffusion(nn.Module):
    def __init__(self, mean_type, noise_schedule, noise_scale, noise_min, noise_max, steps, device, beta_fixed=True):

        self.mean_type = mean_type # MeanType for diffusion: x0, eps
        self.noise_schedule = noise_schedule # the schedule for noise generating
        self.noise_scale = noise_scale # noise scale for noise generating
        self.noise_min = noise_min # noise lower bound for noise generating
        self.noise_max = noise_max # noise upper bound for noise generating
        self.steps = steps
        self.device = device
    ...
    def calculate_for_diffusion(self):
        ...
        self.alphas = alphas
        self.alphas_cumprod = th.cumprod(alphas, axis=0).to(self.device) # 累乘
    ...
    def p_sample(self, model, x_start, cluster_center, steps, sampling_noise=False):
        ...
        for i in indices:
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
            out = self.p_mean_variance(model, x_t, t, cluster_center)
            if sampling_noise:
    ...
    def training_losses(self, model, x_start, sigma, reweight=False):
        batch_size  = x_start.size(0)
        ts, pt = self.sample_timesteps(batch_size, self.device) # algorithm 1, Line 4
        noise = th.randn_like(x_start) # algorithm 1, Line 4
        if self.noise_scale != 0.:
            x_t = self.q_sample(x_start, ts, noise) # eq. 3
        else:
            x_t = x_start

        terms = {}
        model_output = model(x_t, ts, sigma) # reverse process
        target = {
        ...
        if reweight == True:
            if self.mean_type == ModelMeanType.START_X:
                weight = self.coefficient(ts, sigma)
            ...
            weight = th.tensor([1.0] * len(target)).to(self.device)

        terms["loss"] = weight * loss # eq.12 end
    ...
    def p_mean_variance(self, model, x, t, cluster_center):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B, )
        model_output = model(x, t, cluster_center)
    ...
    def coefficient(self, t, sigma):
        self.alphas_cumprod = self.alphas_cumprod.to(t.device)
        self.alphas = self.alphas.to(t.device)

        first_term = 1 / (1 - self.alphas_cumprod[t - 1])
        sec_term = (1 - self.alphas[t]).unsqueeze(0) * sigma.T  * self.alphas_cumprod[t - 1].unsqueeze(0)
        third_term = sigma.T * (1 - self.alphas_cumprod[t]).unsqueeze(0) + (1 - self.alphas[t]).unsqueeze(0) * (1 - self.alphas_cumprod[t - 1]).unsqueeze(0)
        return th.sum(first_term * (sec_term / third_term), dim=0) * 0.5
